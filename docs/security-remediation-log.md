...
## UX-01 架构方案可视化（导师评审反馈）

- **反馈来源：** 导师评审指出交互流程可用，但「架构方案不够直观」，建议把生成的云架构画成拓扑图。
- **问题本质：** 方案只以资源表格呈现，`depends_on` 以分号拼接成文本。读者要在脑中重建依赖图才能判断分层与单点故障，评审成本高。
- **可用前提：** `ArchitecturePlan` 的依赖图在 schema 层已校验为名称唯一、无自环、无未知依赖且无环，可直接绘制，无需额外推断。
- **最小根本修复：** 新增 `src/topology.py`，按 Azure 资源族把资源分入边缘接入/网络/计算/数据/集成/安全与运维六层，输出 Graphviz DOT，由 `st.graphviz_chart` 在浏览器端渲染。
- **依赖边界：** `st.graphviz_chart` 接受 DOT 字符串，浏览器端绘制，**未新增任何 Python 依赖**，不改变锁文件，保持本地闭环。
- **不可信输入处理：** 规划智能体输出按不可信展示数据处理。标签内转义反斜杠与引号，中和 `<`、`>`、`|` 以防改变布局；标识符剔除引号等字符并加位置前缀避免碰撞；所有标识符加引号包裹，保证任意字符下仍为合法 DOT。
- **可读性修正：** 首次实现用 `[^0-9A-Za-z_]` 造 slug，导致中文资源名塌缩为 `res_2______`，导出的 DOT 源码不可读。改用 `\w` 配合 `re.UNICODE` 保留各语种字母，同时保持引号剔除不变。
- **容错：** 渲染失败降级为提示并引导查看资源清单的依赖列，不中断页面。
- **修复后证明：** `tests/test_topology.py` 9 项，覆盖依赖边方向、节点数、引号越界注入、尖括号中和、slug 碰撞、中文可读性与引号包裹；引号转义经故障注入验证确实被捕获。真实 Phi-3 输出端到端生成 4 资源 27 行 DOT。

## UX-02 失败诊断与结构化日志（导师评审反馈）

- **反馈来源：** 导师指出「异常处理有点单薄，模型加载失败时的页面提示不够详细」，建议错误日志更详细以便排查。
- **问题本质：** `AppConfig.validate()` 区分十六种失败模式，但 UI 统一渲染为一行英文原始错误。用户无法区分「文件不存在」与「模型在可信根目录之外」，也不知道该改哪个环境变量。
- **最小根本修复（页面提示）：** 新增 `src/diagnostics.py`，把已知失败映射为原因、编号排查步骤与相关环境变量；原始异常类型、消息与 `__cause__` 保留在可展开区域，未知错误不被吞掉。运行记录只存字符串，故同时提供 `diagnose_text` 供 FAILED 分支复用同一套规则。
- **最小根本修复（日志）：** 新增 `src/run_log.py`，每次结构校验失败记录 run id、角色、阶段、审查轮次、尝试序号、错误类型与解析器给出的精确行列；run 级失败记录已完成轮次。
- **日志数据边界：** 模型输出不可信且可能很大，只记录字符数与单行有界摘要，不落盘完整输出。日志默认经 `NullHandler` 静默，须由侧边栏显式开启；handler 按名替换而非追加，避免 Streamlit 重跑导致重复输出。
- **修复后证明：** `tests/test_diagnostics.py` 15 项，其中九项直接驱动真实 `AppConfig.validate()` 失败路径，确保匹配规则与实际抛出的错误一致而非凭空编写；`tests/test_run_log.py` 7 项覆盖记录字段、长输出不落盘、单行摘要与日志级别。
- **既有测试处理：** 错误呈现移入 ui 模块后，`test_failed_new_attempt_does_not_render_previous_success` 改为断言 `render_failure` 的调用与异常类型，断言强度未降低；重新注入 SEC-10 缺陷仍可使其失败，确认运行隔离保护未被削弱。
- **剩余风险：** 诊断规则按错误消息子串匹配，上游若改写文案会退化为通用提示而非误导；测试直接驱动真实校验路径，可在回归时暴露此类漂移。
