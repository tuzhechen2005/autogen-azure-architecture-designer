...
- 方案以**架构拓扑图**呈现：资源按 Azure 分层（边缘接入、网络、计算、数据、集成、
  安全与运维）分组，依赖关系绘制为有向边，可展开查看并复制 DOT 源码。
- 运行失败时给出**可操作的诊断**：区分模型路径、可信根目录、GGUF 校验、超时、
  结构校验等具体原因，并附排查步骤、相关环境变量和原始错误信息。
- 可选输出**结构化诊断日志**，记录每次结构校验失败的角色、轮次、尝试次数和解析
  错误位置；只保留输出长度与短摘要，不落盘完整模型输出。
